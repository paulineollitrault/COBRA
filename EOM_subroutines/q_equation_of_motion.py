...
class QEquationOfMotion:
    ...
    def __init__(self, operator, num_orbitals, num_particles,
        ...
        # NOTE operator = H
        # NOTE qubit_mapping excitation to qubit; important
    ...
    def calculate_excited_states(self, wave_fn, excitations_list=None, quantum_instance=None):
        ...
        """
        # WAVE_FN: circuit (VQE, Ry etc)
        # EXCITATION_LIST: excitation operators (E_mu) indices in H1 and H2 fermionic operators [the basis]
        ...
        self._operator = op_converter.to_weighted_pauli_operator(self._operator) # NOTE H in weighted Pauli format, important
        self._untapered_op = op_converter.to_weighted_pauli_operator(self._untapered_op) # NOTE tapered vs untapered

        excitations_list = self._de_list + self._se_list if excitations_list is None else excitations_list

        # build all hopping operators NOTE: this changes
        hopping_operators, type_of_commutativities = self.build_hopping_operators(excitations_list) # from the list, build the qubit operators Emu NOTE
        # build all commutators: NOTE this changes
    ...
    # NOTE: the bottleneck -- hopping_operators: list of paulis (first turn them into Paulis, then compute commutators
    # we want the reverse
    ...
    def build_all_commutators(self, excitations_list, hopping_operators, type_of_commutativities):
        ...
        # NOTE: returns weighted Pauli rep of each commutator
        return q_commutators, w_commutators, m_commutators, v_commutators, available_entry

    # NOTE: does not change, the generation of the commutators changes
    ...
    def _build_single_hopping_operator(index, num_particles, num_orbitals, qubit_mapping,
        ...
        if len(index) == 2:
            ...
            i, j, k, m = index
            h2[i, j, k, m] = 16.0
        # turned into a Fermionic operator NOTE
        fer_op = FermionicOperator(h1, h2) # NOTE fermionic operator
        qubit_op = fer_op.mapping(qubit_mapping) # NOTE map to qubit
